...
Remove all binary data, except for the EOF packages used by eofi2tp.
% -----------------------------------------------------------------------------
...
but are padded with zero bytes, if shorter than...
% -----------------------------------------------------------------------------
...
datatype. WE DON'T use variable length strings, but varialbe length data!
% -----------------------------------------------------------------------------
...
% -----------------------------------------------------------------------------
\subsection{Peer fingerprint (pfpr)}
The peer fingerprint is a 40-byte fixed length string.
% -----------------------------------------------------------------------------
\subsection{Padding data}
Should be random data. NOT NEEDED, remove.
% -----------------------------------------------------------------------------
\subsection{EOF Commands}
...
Connections to EOFi can be done via Unix-sockets (named Unix sockets) to
~/.ceof/
...
% -----------------------------------------------------------------------------
\subsubsection{Listening transport protocol (LTP)}
The listening transport protocol uses the destination parameter as the
address to listen to.
% -----------------------------------------------------------------------------
\subsubsection{Transport protocol (TP)}
The transport protocol uses the destination parameter as the address to send to.
% -----------------------------------------------------------------------------
...
\hline
Destination & URL without scheme as zero-padded 128-byte fixed-length string & The \emph{proto:} part is stripped of & 127.0.0.3:42\\
\hline
...
Note that \textbf{.} in this example stands for a zero byte.
\begin{verbatim}
1000127.0.0.3:42....................................................................................................................10
APRILFOOLS
\end{verbatim}
% -----------------------------------------------------------------------------
\subsection{1001: Set listen address}
\begin{longtable}{|c|c|c|c|}
\caption{Command 1001 parameters}\\
\hline
\textbf{Parameter} & \textbf{Type} & \textbf{Description} & \textbf{Example}\\
\hline
Listen Address & URL without scheme as zero-padded 128-byte fixed-length string & The \emph{proto:} part is stripped of & 127.0.0.3:42\\
\hline
\end{longtable}
% -----------------------------------------------------------------------------
\subsubsection{Example}
...
After the "`0022"' follow 128 bytes for the peer name
...
After the "`0023"' follows a 128 bytes fixed length string for the peer name.
Answers from ceof:
\begin{itemize}
...
After the "0032" follow 128 bytes for the channel name and after
