...
EOF knows about
\begin{itemize}
\item transport protocol dependent, encrypted packets ("`postcard"')
\item plain encrypted ("`Onion packets"') and
\item unencrypted ("`plaintext packets"') packets.
\end{itemize}
...
This document specifies the commands send from crypto to and from
EOFi\footnote{the central EOF-1 application} to the crypto.

% -----------------------------------------------------------------------------
\subsection{Connection}
The crypto is started by EOFi at startup and communicates through stdin
and stdout.
% -----------------------------------------------------------------------------
\subsection{Commands}
...
\end{itemize}
% -----------------------------------------------------------------------------
\subsection{3001: Deregister crypto}
...
After the \textbf{crypto} send \textit{pck\_len},
...
If the response is \textit{1030}, \textbf{crypto} should send the
pcaket, otherwise this session is finished and \textbf{crypto}
...
After \textbf{EOFi} send \textit{msg\_len}, crypto must respond with
either
\begin{itemize}
